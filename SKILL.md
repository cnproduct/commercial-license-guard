...
3. **授权码与店铺 1:1 永久强锁定与单窗口防串店红线 (Store-to-License Mutex Architecture)**：
   - **一店一码 1:1 强绑定**：商业授权码（License Key）与 Wildberries 店铺（OpenAPI Token / 卖家ID）产生 1:1 永久强锁定（¥600/店铺/年，365天有效）。一个授权码只能绑定一个特定店铺，严禁通过轮播换绑白嫖，跨店上架强制阻断并提示开通新店铺授权。
   - **Conversation ID 的轻量定位**：Conversation ID 不作为资产所有者，仅作为收银台秒级自动免密流转管道与窗口内单店防串店沙箱。
   - **单窗口换店配额**：商业会话窗口最多仅允许更换 1 次店铺（`switch_count <= 1`），换店后永久锁定。
   - **紧凑 3cm×3cm 二维码规范**：聊天界面内必须使用显式行内 CSS 约束的 `<img>` 标签输出 100×100 px 二维码，杜绝屏幕全宽拉伸。
4. **双引擎平滑容灾红线**：
